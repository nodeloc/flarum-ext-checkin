...
app.initializers.add('gtdxyz-checkin', () => {
  ...
  app.checkinClick = function() {
    grecaptcha.ready(function() {
        grecaptcha.execute('6LdGGOIpAAAAACV5Va6DYtL8xkUvJl78yexKGKBx', {action: 'checkin'}).then(function(token) {
            performCheckin(token);
        });
    });
  };

  app.randomcheckinClick = function() {
    grecaptcha.ready(function() {
        grecaptcha.execute('6LdGGOIpAAAAACV5Va6DYtL8xkUvJl78yexKGKBx', {action: 'checkin'}).then(function(token) {
  ...
  function performCheckin(recaptchaToken) {
    ...
    app.session.user.save({allowCheckin:false,checkin_days_count:checkin_days_count+1,checkin_type: 'N', recaptchaToken: recaptchaToken})
    .then(() => {
  ...
  function performRCheckin(recaptchaToken) {
    const checkin_days_count = app.session.user.attribute("checkin_days_count");
    const checkinReward = app.forum.attribute("checkinReward");
    const checkinSuccessType = app.forum.attribute("checkinSuccessType");
    const checkinSuccessText = app.forum.attribute("checkinSuccessText");
    ...
    let error_text = app.translator.trans('gtdxyz-checkin.forum.failed')+" "+app.translator.trans('gtdxyz-checkin.forum.try-again-later');
    app.session.user.save({allowCheckin:false,checkin_days_count:checkin_days_count+1,checkin_type: 'R', recaptchaToken: recaptchaToken})
      .then(() => {
        if(checkinSuccessType===1){
          let rewardText = "";
          if(checkinSuccessText!==""){
